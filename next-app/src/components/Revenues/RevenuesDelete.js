...
function RevenuesDelete({ reloadPage }) {
    ...
    const deleteRevenue = async () => {
        try {
            const response = await fetch(`http://localhost:3008/api/revenues/${dropdownId}`, {
              method: "DELETE",
              headers: {
            ...
            <Segment> 
                <Form>
                    ...
                    <br />
                    {
                        dropdownId !== "choice" ?
                            (
                                <Button onClick={deleteRevenue} type='submit'>Delete</Button>
                            )
                        :
                            null
                    }
