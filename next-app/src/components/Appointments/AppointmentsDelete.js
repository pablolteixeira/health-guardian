...
function AppointmentsDelete({ reloadPage }) {
    ...
    const deleteAppointment = async () => {
        try {
            const response = await fetch(`http://localhost:3008/api/appointments/${dropdownId}`, {
              method: "DELETE",
              headers: {
            ...
            <Segment> 
                <Form>
                    ...
                    <br />
                    {
                        dropdownId !== "choice" ?
                            (
                                <Button onClick={deleteAppointment} type='submit'>Delete</Button>
                            )
                        :
                            null
                    }
